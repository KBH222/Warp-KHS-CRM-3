...
// This will be synced as part of the sync data
const getToolsData = (): ToolsData => {
  const syncData = useSyncStore.getState();
  return syncData.toolsData || {
...
const saveToolsData = (data: ToolsData) => {
  useSyncStore.setState({ toolsData: data });
  // Also save to localStorage for persistence
  localStorage.setItem('khs-tools-sync-data', JSON.stringify(data));
};
...
const KHSInfoLocal = () => {
  ...
  const [toolsData, setToolsData] = useState<ToolsData>(getToolsData());
  ...
  // Load saved data on mount and listen for sync updates
  useEffect(() => {
    // Load from sync store or localStorage
    const savedData = localStorage.getItem('khs-tools-sync-data');
    if (savedData) {
      try {
        const parsed = JSON.parse(savedData);
        setToolsData(parsed);
      } catch (e) {
        console.error('Failed to parse saved tools data:', e);
      }
    }
    ...
    return (
      <div style={{ padding: '20px' }}>
        ...
        <div style={{
          marginBottom: '16px',
          padding: '8px 12px',
          backgroundColor: syncStatus.syncInProgress ? '#FEF3C7' : '#D1FAE5',
          color: syncStatus.syncInProgress ? '#92400E' : '#065F46',
          borderRadius: '6px',
          fontSize: '14px',
          display: 'flex',
          alignItems: 'center',
